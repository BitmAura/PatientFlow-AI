...
type TwentyOneButtonProps = React.ButtonHTMLAttributes<HTMLButtonElement> & {
  ...
  /** Render as child (e.g. Next.js `Link`) for navigation without nested buttons */
  asChild?: boolean
}
  ...
  ({ className, tone = 'primary', children, asChild = false, type, ...props }, ref) => {
    ...
    return (
      <Comp
        ...
        className={cn(
          'group relative inline-flex h-12 min-h-[48px] items-center justify-center rounded-xl border px-6 text-sm font-semibold tracking-wide shadow-lg transition-all duration-200 hover:-translate-y-0.5 active:scale-[0.99]',
          toneClasses,
          className
        )}
        {...props}
