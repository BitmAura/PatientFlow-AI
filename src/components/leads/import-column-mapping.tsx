...
import { LEAD_COLUMN_MAPPINGS, autoDetectLeadMappings } from '@/lib/import/lead-column-mappings'
...
export function ImportLeadColumnMapping({ 
  headers, 
  sampleData, 
  mappings, 
  onMappingChange 
  ...
  React.useEffect(() => {
    const detected = autoDetectLeadMappings(headers)
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-6">
        {LEAD_COLUMN_MAPPINGS.map((field) => {
            ...
            <Card key={field.value} className={field.required && !selectedHeader ? "border-red-200 bg-red-50/50" : ""}>
              <CardContent className="p-4 grid md:grid-cols-3 gap-4 items-center">
                ...
                <Select
                    ...
                    onValueChange={(val: string) => handleMap(field.value, val === "ignore" ? "" : val)}
                  >
